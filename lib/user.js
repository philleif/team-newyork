...
const verifyPhone = async (user, code) => {
  try {
    if (user.phone.verifyCode === code) {
      ...
      await client.messages.create({
        body:
          "I'll be sending you action alerts around issues and community events. Plus, I'll help you write letters to your representative.",
        to: user.phone.number, // Text this number
        from: process.env.TWILIO_NUMBER // From a valid Twilio number
      })
      ...
      await client.messages.create({
        body:
          "You can also submit issues or community news by saying 'submit'. Selected submissions will be sent as notificaitons to the whole team. 👫👭👬",
        to: user.phone.number, // Text this number
        from: process.env.TWILIO_NUMBER // From a valid Twilio number
      })
