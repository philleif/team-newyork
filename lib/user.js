...
const refreshTokens = async user => {
  try {
    user.tokens.letter = md5(
      Math.floor(Math.random() * 900000 + 100000)
    )
    user.tokens.passwordReset = md5(
      Math.floor(Math.random() * 900000 + 100000)
    )
...
const verifyPhone = async (user, code) => {
  try {
    if (user.phone.verifyCode === code) {
      ...
      await client.messages.create({
        body: "I'll be sending you action alerts around issues and community events.\
      ...
      await client.messages.create({
        body: "You can also submit issues or community news by saying 'submit'.\
